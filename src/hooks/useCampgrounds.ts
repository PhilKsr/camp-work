  ...
  return useQuery<CampgroundGeoJSON>({
    ...
    queryFn: async () => {
      try {
        ...
        const features = (data || []).map((row: { id: string; name: string; type: string; location: string; address: string | null; website: string | null; phone: string | null; email: string | null; rating: number | null; features: string[]; coverage_level: string; opening_hours: string | null; fee: boolean | null; capacity: number | null; source: string; osm_id: string | null; updated_at: string }) => {
          // Parse PostGIS POINT
